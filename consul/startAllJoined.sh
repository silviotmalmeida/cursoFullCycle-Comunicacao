...
docker exec consulserver02 sh -c "consul agent -server -bootstrap-expect=3 -node=consulserver02 -bind=10.5.0.3 -data-dir=/var/lib/consul -config-dir=/etc/consul.d -retry-join=10.5.0.2" &

echo "Subindo o consulserver03..."
docker exec consulserver03 sh -c "consul agent -server -bootstrap-expect=3 -node=consulserver03 -bind=10.5.0.4 -data-dir=/var/lib/consul -config-dir=/etc/consul.d -retry-join=10.5.0.2" &
